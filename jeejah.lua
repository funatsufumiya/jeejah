...
local eval = function(sandbox, code)
   ...
   if(sandbox) then setfenv(chunk, sandbox) end
...
local handle = function(conn, handlers, sandbox, msg)
   ...
   elseif(handlers[msg.op]) then
      d("Custom op:", msg.op)
      handlers[msg.op](conn, msg, sandbox)
   else
